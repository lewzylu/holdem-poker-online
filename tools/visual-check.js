...
for (const d of DEVICES) {
  ...
  /* ---------- 竖屏：只验证引导，牌桌本来就是隐藏的 ---------- */
  if (d.portrait) {
    ok(g.rotate !== 'none', '竖屏显示旋转引导（display=' + g.rotate + '）');
  ...
  if (withInfo.length) {
    ...
    withInfo.forEach(s => s.parts.forEach(p => {
      if (hit(s.hole, p)) { press++; worst = Math.max(worst, Math.min(s.hole.bt, p.bt) - Math.max(s.hole.y, p.y)); }
    }));
